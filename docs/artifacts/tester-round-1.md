...
| #2 (Andrea), 2026-07-23, post-source-audit | enhancement request — accepted (P2 → done same day) | Audited the source (seed gen, wire, transport, deps, build.rs — "clean and careful"); the remaining gap: releases aren't reproducible, so the prebuilt exe can't be tied to the audited source. Asked for pinned toolchain, --locked, timestamp normalization, documented recipe. Will build from tag `v0.1.0-prealpha-c` meanwhile; asked (1) do repo docs/manifest match the package, (2) exact rustc + flags of the -c cut | LANDED `ba938f0`: empirical diagnosis at the tag found the PE link timestamp is the ONLY nondeterminism (no embedded build paths); with `-Wl,--no-insert-timestamp` two builds are byte-identical (cli `426943b7…`, manifest `e0a1a505…`) — rust-toolchain.toml (rustc 1.96.1), `--locked` + RUSTFLAGS in build-release.sh, BUILDING.md recipe + honest status (≤ -c releases won't byte-match; NEXT cut is the first reproducible release). Answers: (1) yes — repo@tag ↔ package byte-identical (guide `d7bdaad5…`, v3.manifest `a63c9b4f…`); (2) rustc 1.96.1 stable windows-gnu, WinLibs GCC 16.1.0 ucrt-posix-seh r2, `cargo build --release --features bitcoind` (pre---locked), CC=gcc AR=ar |

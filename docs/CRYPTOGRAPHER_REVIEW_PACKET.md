...
| `src/chain/mod.rs` | `SimChain` (consensus *physics* only — no script/sig execution) + dual-source chain-view logic. **Read the limitations, Section 5.** |
...
| Relative-vs-S reasoning: on-chain CSVs are BIP68 relative (consensus); S+Δ are policy | v3.13 Map L176/L181 vs L166/L171, L182 | `chain/mod.rs:147–160` (tip−funding≥CSV); `escrow.rs:53–61` `from_height`; `refund.rs:54–56` `csv_maturity = s_height + csv_blocks` | **partial** | `chain/mod.rs::csv_spend_is_rejected_until_matured`, `::double_spend_of_confirmed_output_is_rejected` |
...
| Completion output = exactly D to fresh dest; Δ_fee→miner; single value output + non-dust anchor, no child | v3.13 L38,163–170,201 | `txbuild.rs` `build_completion` (1 in; `[TxOut(D→dest), anchor_output(anchor_sats)]`; positive-fee guard) | implemented | `failure_checklist.rs:1229–1231` (BOTH completion outputs == D); Setup `:1138–1140` (escrow_amount + non-dust anchor) |
| Refund output = exactly D, above dust (scheme (a): settlement_fee + anchor consumed from the escrow) | v3.13 L39,173–180 | `txbuild.rs` `build_refund` (guard `out+anchor < escrow_amount` ⇒ positive fee); `refund.rs:274–279` passes `tier_d_sats`; dust floors = `params.validate()` (tier_d ≥ 1000) + the `chain::policy` dust gate on broadcast | implemented (scheme (a)) | `failure_checklist.rs`, `refund.rs` |
...
| Fee = input − output, saturating (arithmetic totality) | v3.13 L249; Req6 totality | `chain/mod.rs:138–163` (`saturating_add`/`sub`); height/deadline u64 saturating `params.rs:56,114–118` | implemented | `chain/mod.rs:419,430`; `params.rs:131` (no panic at u32::MAX) |
...
| Deterministic race reconciliation; wallet never self-double-spends | v3.13 §Refund b5 | chain layer: `chain/mod.rs:129` `broadcast` (rejects 2nd spend of confirmed `:178–180`, CSV maturity `:148–160`); surfaced `refund.rs:176` | implemented (SIM physics, not consensus) | `double_spend_of_confirmed_output_is_rejected`, `refund_completion_race_resolves_deterministically` |

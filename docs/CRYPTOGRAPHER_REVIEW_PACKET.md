...
| Claim-delay bound: even at max delay, SL confirms before S+Δ_late | v3.16 Req6 L66; v3.13 Map L171, L251 | `params.rs:114–118` `max_claim_delay`; sampler `state_machine.rs:698–709`; consumed `:831–854` | implemented | `two_party_exchange_extract_and_claim`, `claim_delay_sampler_respects_boundaries` |
...
| Escrow input = D + Δ_fee (Setup spends UTXO whole, no change) | v3.13 L37; Map L153/L158 | amount threaded via `txbuild::escrow_prevout:41–46`, `chain::fund_with_amount:75`; callers set `tier_d + delta_fee` | **partial** (no Setup builder; funding injected abstractly) | `tests/taproot_swap.rs:68`, `failure_checklist.rs:118,600` |
...
| Every contract tx TRUC/v3 | v3.13 L4,27,74,197 | `txbuild.rs:28` `Version(3)`, applied `:62,:98` | **partial** (Setup unbuilt; SimChain enforces no TRUC package-relay semantics) | `txbuild.rs:191`, `refund.rs:277` |
...
| Self-verifying dual-source chain view; disagreement → wait-or-abort | v3.13 What's-New; Threat "eclipse" | `chain.rs:210` `ChainSource`/`is_self_verifying:218`; `:263` `DualSourceChainView` (`new` needs ≥1 SV `:270`); `verified_*` Err on disagreement `:294/:305/:317`; `ChainView` conservative collapse `:329–341` | **partial** — `is_self_verifying` is a **bool label**; no real BIP157/158 filter or PoW-header validation (see 5) | `dual_source_requires_at_least_one_self_verifying`, `eclipse_all_api_path_is_defeated_by_the_self_verifying_source`, `await_funded_refuses_under_eclipse_but_proceeds_on_agreement` |
...
3. **The skew anchoring IS tested; a full skewed end-to-end swap is not.** `failure_checklist.rs::cofunding_skew_anchors_claim_to_swept_escrow_not_s` drives `await_funded` at maximum skew (`f_sh = S − cofunding_window`), asserts `sweep_escrow_height == f_sh`, and proves the correctly-anchored claim confirms before `f_sh + Δ_late` while the *old* S-anchored bound would have breached it (a genuine regression guard for the critical fix). What remains untested is a full **six-message exchange + broadcast + extract + claim** driven at skew ≠ 0 — the exchange harness uses `funded_manual` (`state_machine.rs`), which hardcodes `sweep_escrow_height = s_height` (skew = 0). Recommend adding that end-to-end skewed-swap test before model-checking.
...
`canonical_key_agg` (`state_machine.rs:220`) is the authoritative sort+reject-equal helper, but the same lexicographic sort is **re-implemented inline at four other sites** (`escrow.rs:183`, `txbuild.rs:198`, `refund.rs:260`, signing tests `:331`). Consistent today, but a **divergence risk** if one site changes. Recommend consolidating on the single helper. Also confirm `swap_session_id` / `role_seed` derivations use the **same** sort (cross-consistency asserted by spec; derivations partly outside these files — and see 5, `swap_session_id` derivation is itself a stub).
...
|---|---|---|---|
| **TEK / GCM at-rest custody** (HKDF→TEK, AES-256-GCM, enclave-wrapped) | **stub — not built** | Zero HKDF/GCM/AES/TEK/enclave code. Persistence that exists is **plaintext**: SL possession record (`state_machine.rs:616`) and lease file (`signing/mod.rs:97`). Money-bearing secrets (`t`, MuSig2 seckeys) are **never persisted** — only in volatile memory — so the fatal leak class is avoided by non-persistence. Records contain no secret nonces/keys; integrity covered by re-verification on restore, **confidentiality is not.** | MEDIUM — the audit-list "TEK/GCM nonces" item has **nothing to review yet.** |
...
| **Setup-tx construction (zero-change)** | **stub** | Only `build_completion` + `build_refund` exist; **no `build_setup`.** Escrow funding is synthetic (`chain.fund`/`fund_with_amount`). "Whole-UTXO spend, no change" + SL-first Setup broadcast **neither built nor tested against real tx construction.** Completion/refund that *are* built carry TRUC v3 + anchor. | LOW for settlement crypto (the escrow output the Setup pays to IS reviewed); fee/funding/change arithmetic only partly exercised. |
...
| Δ_fee signing/versioning; SL-first order + funding jitter; `swap_session_id`=SHA256(pub‖pub) & `swap_key`=HKDF; real Tor broadcast | **stub/deferred** | Δ_fee is a plain field (no manifest signing/version-gating → equal-Δ_fee anonymity **cannot be enforced by this crate**). SL-first order + jitter absent (privacy/grief, not theft). `swap_session_id` is **passed in** (no SHA256-of-pubkeys derivation) — anti-divergence for the id is by-assumption; `swap_key`/HKDF fully absent. Tor/multi-peer/Dandelion entirely absent (comments only). | LOW–MEDIUM — `swap_session_id` derivation should be reviewed **as design intent** (a divergent derivation breaks the lease and any swap_key material). |
...
**Crash safety:**
- `state_machine.rs:1138` `sl_restores_possession_after_crash_and_claims` — drops in-memory `Possessing`, rebuilds from disk in a fresh process, extracts + claims.
- `:1173` `corrupt_possession_record_is_rejected` — flipped byte → restore Err.

...
| `src/tx/` (`escrow.rs`, `txbuild.rs`) | Taproot escrow + refund leaf, tweak-equality guard, TRUC/v3 + non-dust P2A-anchor completion/refund builders (scheme (a), §4.98). |
| `src/crypto/validate.rs` | Curve-point / scalar / nonce / partial / final-sig validation gate (the deserialization hard gate). |
| `src/chain/` (`mod.rs`, `policy.rs`) | `SimChain` (consensus *physics* + modeled Core relay policy `policy.rs` §4.98 — no script/sig execution) + dual-source chain-view logic. **Read the limitations, Section 5.** |
...
|---|---|---|---|---|
| Complete pre-sig = aggregate of **both** partials over agreed nonce, adaptor-shifted by `T`, verifying against `(R+T,P,m)`; not constructible from a lone partial (closes v3.11 bug) | v3.13 Ph5 L5,26,86–88; audit L249 | `signing/mod.rs:300` `assemble_complete_presig` (→`:317`); `crypto/adaptor.rs:88–134` `CompletePreSig`+`new()` (pub(crate)); verify `adaptor.rs:143`→`:148` | implemented | `state_machine.rs:1116` (end-to-end both legs, independent BIP340 verify `:1015`) |
| Exact Phase-5 message order, each verified before next sent; any failure aborts both sessions | v3.13 Ph5 L82–89,128 | `state_machine.rs:455` `run_adaptor_exchange` (commit `:490/:491`; reveal `:494/:498`; re-check `:505`; SH `T:527`/`ShPartials:532`/recv `:538`; SL `T:564`/`ShPartials:568`/enabling `:627`); tags `wire/mod.rs:29–47` | implemented | `state_machine.rs:1116`; wire round-trip + unknown-tag reject `wire/mod.rs:187,205` |
| Concurrent-session interlock: both sessions' nonces committed before either revealed (Wagner/Drijvers) | v3.13 L23,83–84,101 | `signing/mod.rs:224` `commit_and_reveal`; wire enforcement `state_machine.rs:485–509`; `sign_partial` refuses if `!committed_both_sessions` (`:193`) | implemented | `signing/mod.rs:352,376`; wire-mismatch branch `state_machine.rs:505` adversarially covered by `failure_checklist.rs::equivocated_nonce_reveal_is_rejected:829` (see 4.3) |
| Possession gate **G1**: SL releases enabling partial only after verified complete pre-sig for Comp→SH, bound to this swap | v3.13 L88,23,252 | assemble `:584`→verify `:588`; own-leg `:592/:596`; `PossessionWitness` mint `:623`; `release_enabling_partial:302` (sole tag-0x04 sender, swap-id check `:308`) | implemented | `state_machine.rs:1162` (definitive G1 negative: corrupt partial → no release, no record) |
| G1 crash-safety: persist pre-sigs **before** releasing enabling partial | v3.13 Ph5 L89; SM L99,102,130–131 | `state_machine.rs:606`→`write_possession_record:669` (atomic tmp+rename `:721–724`) **before** mint/release; restore `:793` re-verifies both legs `:835–836`; serde `adaptor.rs:189/:217/:259` | implemented | `state_machine.rs:1264` (fresh-process restore+claim); `:1300` (corrupt record → Err) |
| Extraction correctness: recover `t = s_final − s_hat`; never return a wrong secret (`t·G == T`) | v3.13 Ph5 L92; audit L249 | `adaptor.rs:157` `extract_secret` (reveal `:160`; zero-check `:162–167`; `t·G==T` guard `:168–170`); driver `state_machine.rs:921` (SL-only `:936`) | implemented | `state_machine.rs:1351` (unrelated final sig → Err); `:1116/:1153` (extracted `t` proven correct by independent BIP340 verify) |
...
| INV-2 no session survives restart → ABORT_REFUND | v3.16 Req3; v3.13 L99,96–99 | `signing/mod.rs:131–142` (no (de)serialize); `begin():152–178` sole constructor | implemented | `signing/mod.rs:390–412` (crash via `mem::forget`, restart refused) |
| INV-3 single-signer lease keyed to swap_session_id | v3.16 Req3; v3.13 L224 | `signing/mod.rs:66–121` `SingleSignerLease`; atomic `create_new` `:92–108`; `Rc<..>` required at construction | **partial** (local-FS prototype of a cross-device primitive) | `signing/mod.rs:337–348,390–412` |
| INV-4 fresh nonce every session/retry; BIP327 NonceGen w/ fresh randomness only; deterministic forbidden | v3.16 Req3+Req1; v3.13 L69,83,98 | `signing/mod.rs:152–178` (OsRng seed `:160–163`→`SecNonce::generate:166`, domain tag `b"newkey-v3.16"`); `sign_partial(self)` consumes session `:192` | implemented | `signing/mod.rs:415–424` (identical inputs → distinct nonces) |

### 3.3 Timelock & deadline arithmetic

| Requirement | Spec | Code location | Status | Test |
|---|---|---|---|---|
| Δ_late = Δ_early + margin, **strictly** later; ordering invariant inviolable | v3.16 Req6 L59; v3.13 L70 | `params.rs:81` `delta_late()` (u64); `validate():122` rejects margin==0 (`:124–126`); ≤u16::MAX `:202` | implemented | `params.rs::provisional_defaults_validate`, `::ordering_violations_are_rejected_not_panicked` |
| Δ_buffer real deadline strictly inside (0, Δ_early); SH confirms Comp→SH by S+Δ_early−Δ_buffer | v3.16 Req6; v3.13 SM L112, Map L166 | `params.rs:128–130` validate; gate `state_machine.rs:891–897` (`>=` refuses boundary) | implemented | `broadcast_gate_boundary_and_receipt` (S+120 refused, S+119 allowed) |
| Broadcast gate = evidence not bool: runway **AND** watchtower armed w/ *this* refund | v3.13 SM L111–113; policy G2 | `state_machine.rs:879–907`; watchtower check `:898`; `WatchtowerReceipt` `refund.rs:90–115` | implemented | `broadcast_gate_boundary_and_receipt` (wrong receipt → Err) |
| Claim-delay bound: even at max delay, SL confirms before S+Δ_late | v3.16 Req6 L66; v3.13 Map L171, L251 | `params.rs` `max_claim_delay`; sampler `state_machine.rs:769–780`; consumed `:916–926` | implemented — **SEMANTICS TIGHTENED 2026-07-07 (⚠ post-packet change inside the frozen timelock surface — re-review):** the budget now ends at `deadline − allowance − 1` (extra −1). Rationale: under BIP68 the SH refund is includable in block `anchor+Δ_late` ITSELF, so a claim budgeted to confirm AT the deadline shares a block with the live refund — the boundary is the race. The change strictly SHRINKS the granted delay (safe direction), and the wallet-layer manifest enforces the matching static bound (`delay_max < margin+Δ_buffer−cofund−allowance`, strict). Checklist row 5's property is now strict (`<` not `<=`). | `two_party_exchange_extract_and_claim`, `claim_delay_sampler_respects_boundaries`, row-5 strict property |
| **Co-funding-skew anchor fix**: SL claim deadline anchored to swept escrow height `f_sh`, not S | v3.13 L21/L75; v3.16 L59 | `params.rs:215–238`; `Funded.sweep_escrow_height` `state_machine.rs:100–104`, set `:399–405`; used `:953–955` | **divergence-as-hardening** (see 4.1) | `cofunding_skew_anchors_claim_to_swept_escrow_not_s:737` + `full_swap_under_cofunding_skew:938` — both drive real `await_funded` at skew=cofunding_window (see 4.1 pt 3) |
| Escrow orientation: SL-funded→Δ_early, SH-funded→Δ_late; leaves for both orderings, unused discarded | v3.13 L70; Map L176/L181 | `escrow.rs:85–117` builds **one** CSV leaf; `txbuild.rs:115–156` uses `csv_blocks()`. No role→{Δ_early\|Δ_late} mapper in production. | **stub** (primitives exist; policy binding absent) | tests hard-code the pairing (src unit tests pass literals 216/144; integration tests hand-assign `delta_early`/`delta_late()` per escrow), not role-derived. No orientation assertion. |
| Relative-vs-S reasoning: on-chain CSVs are BIP68 relative (consensus); S+Δ are policy | v3.13 Map L176/L181 vs L166/L171, L182 | `chain/mod.rs:264–281` (tip−funding≥CSV); `escrow.rs:53–61` `from_height`; `refund.rs:57–59` `csv_maturity = s_height + csv_blocks` | **partial** | `chain/mod.rs::csv_spend_is_rejected_until_matured`, `::double_spend_of_confirmed_output_is_rejected` |

### 3.4 KeyAgg coefficients & canonical ordering (rogue-key safety)

| Requirement | Spec | Code location | Status | Test |
|---|---|---|---|---|
| KeyAgg uses BIP327 per-key coefficients (rogue-key-safe) | v3.13 L22,68,234; v3.16 Req1 | `musig2::KeyAggContext::new` now centralized to `state_machine.rs:240` (inside the shared `canonical_key_agg`) + `adaptor.rs:249`; all escrow/txbuild/refund/signing aggregation routes through the shared helper (`escrow.rs:182`, `setup.rs:224`, `txbuild.rs:229`, `refund.rs:263`, `signing/mod.rs:330`) — see §4.7 | implemented (inherited from pinned dep) | `escrow.rs:186–205`; **no rogue-key attack test** (property inherited from library audit) |
| Canonical lexicographic party ordering for KeyAgg (order-dependent) | v3.13 L22,44,68,234 | `state_machine.rs:227–233` `canonical_pair` (sort + reject a==b) — **single source of truth**; `canonical_key_agg:239` / `canonical_internal_key:246` build on it; all former inline sites now call it | implemented (**consolidated** — the 4-duplicate-sort divergence-risk is RESOLVED, see §4.7) | incidental via escrow/signing tests |
...
| Funded-key==signed-key: tweaked aggregate byte-equals escrow output key else abort | v3.13 L249; v3.16 Req2 | `escrow.rs:156–169` `taproot_tweaked_keyagg` (`with_taproot_tweak`→compare→`TweakMismatch`) | implemented | `escrow.rs:198–201` (tweaked x-only == output key) |
...
| Requirement | Spec | Code location | Status | Test |
|---|---|---|---|---|
| Δ_fee fixed, signed, versioned, manifest-distributed, not user-editable | v3.13 L11,35 | `params.rs:38` field (`:68` default 5_000; `validate():149–151` rejects 0); signed inside the manifest body `wallet/manifest.rs:267–277` (BIP340 tagged-hash verify `:364–384`; monotonic version floor `:415–420`; swap refusal `:555`) | **implemented (manifest layer)** — Δ_fee + the scheme-(a) fee components (setup_fee, anchor; settlement fee derived) are signed, version-gated, fields private, AND the swap-persistence entry point now enforces params==manifest: `SwapEngine::record_funding` rejects any params that diverge from the signed manifest before persisting (`engine.rs` guard `&params != self.manifest.current().params()` ⇒ `Error::Validation`); `run_exchange` then pins that snapshot immutable across puts. Residual: no production driver assembles a full swap end-to-end yet (rank 4) — `orchestrator.rs:82` `from_manifest` snapshots manifest params but has no production caller — so the reject branch is exercised only by test until that driver lands. | `provisional_defaults_validate`; `record_funding_rejects_params_off_manifest`; manifest suite (§5) |
| Escrow input = D + Δ_fee (Setup spends UTXO whole, no change) | v3.13 L37; Map L153/L158 | `tx::setup::build_setup` — whole pre-encumbrance (D+Δ_fee) UTXO → escrow, single input, `[escrow(D+Δ_fee−setup_cost), anchor(anchor_sats)]`, no change (scheme (a), §4.98: the escrow OUTPUT is reduced by setup_cost; the difference is the Setup's positive standalone-relay fee); escrow outpoint = setup_txid:0 | **implemented** (scheme-(a) output value — see §4.98) | `setup_is_zero_change_and_signs_the_pre_encumbrance`, `full_swap_from_real_setup_funding` |
| Completion output = exactly D to fresh dest; Δ_fee→miner; single value output + non-dust anchor, no child | v3.13 L38,163–170,201 | `txbuild.rs` `build_completion` (1 in; `[TxOut(D→dest), anchor_output(anchor_sats)]`; positive-fee guard; the fee actually paid is `settlement_fee` — Δ_fee's residual after setup_fee + 2·anchor, §4.98) | implemented | `failure_checklist.rs:1229–1231` (BOTH completion outputs == D); Setup `:1138–1140` (escrow_amount + non-dust anchor) |
| Refund output = exactly D, above dust (scheme (a): settlement_fee + anchor consumed from the escrow) | v3.13 L39,173–180 | `txbuild.rs` `build_refund` (guard `out+anchor < escrow_amount` ⇒ positive fee); `refund.rs:272–282` passes `tier_d_sats`; dust floors = `params.validate()` (tier_d ≥ 1000) + the `chain::policy` dust gate on broadcast | implemented (scheme (a)) | `failure_checklist.rs`, `refund.rs` |
| Every contract tx TRUC/v3 | v3.13 L4,27,74,197 | `txbuild::TRUC_VERSION = Version(3)` on completion/refund/`setup::build_setup`; `chain::policy` MODELS TRUC/BIP431 (v3-only unconfirmed chains, transitive ≤1 ancestor, 1 child, child-size cap) + 1P1C package relay, enforced by `SimChain::broadcast`/`submit_package` | implemented (policy modeled in-sim; real-node package relay deferred to testnet) | `chain/mod.rs` `truc_*`, `truc_ancestor_count_is_transitive_not_per_input`, `zero_fee_ephemeral_dust_parent_relays_only_in_package` |
| TRUC/v3 non-dust P2A anchor (scheme (a) ≥ 240 sat), CPFP-spent only on a real fee spike w/ consent, else unspent | v3.13 L12,40–41,74,197–203 | anchor `OP_1 0x4e73` on every contract tx; CPFP child `tx::backstop::build_cpfp_bump` (1P1C, anchor+reserve); consent `acknowledge_linkage`/`LinkageAck` + `watchtower_driver::backstop_decision` (silent refund vs consent-gated completion) | **partial** (builder + consent + routing built & tested; NO production loop wires the bump yet; real package acceptance deferred to testnet) | `tx/backstop.rs`, `watchtower_driver.rs` tests |
| Congestion backstop: opt-in CPFP from reserve UTXO; off by default | v3.13 L40–41,108,140 | congestion min-fee gate + RBF (`chain/mod.rs`); reserve CPFP builder `tx::backstop::build_cpfp_bump`; reserve leasing `ledger::lease_reserve`; consent `LinkageAck`; package acceptance `SimChain::submit_package` (judged on package feerate) | **partial** (builder + reserve lease + consent + package-feerate acceptance built & tested; production loop wiring + real-node relay deferred) | `tx/backstop.rs`, `chain/mod.rs` package tests, `watchtower_driver.rs` |
| Onboarding auto-split → D+Δ_fee UTXOs + single change (the ONLY change in lifecycle) | v3.13 L10,58,192–195 | `tx::setup::build_onboarding_split:129` + `wallet/ledger.rs` `split_deposit:492` (≤64 outputs, shuffled change position, sub-dust folds to fee, randomized confirmation-anchored delay) — see §5 ledger+onboarding row | **implemented (wallet layer)** | `tests/onboarding.rs::deposit_split_bump_delay_setup_escrow_end_to_end:35` + ledger split unit tests |
| Fee = input − output, saturating (arithmetic totality) | v3.13 L249; Req6 totality | `chain/mod.rs:260,531–532` (`saturating_add`/`sub`); height/deadline u64 saturating `params.rs:81,233–237` | implemented | `chain/mod.rs:911,922`; `params.rs:319` (no panic at u32::MAX) |

### 3.7 Watchtower bundle & deadline discipline (G2), completion-supersedes, dual-source view

| Requirement | Spec | Code location | Status | Test |
|---|---|---|---|---|
| Pre-armed refund exists on disk before any completion broadcast | v3.13 Pre-armed/§Deadline b1 | `refund.rs:37` `arm`; `:64` `from_signed_tx`; input at `state_machine.rs:159`, threaded `:558/:640` | implemented (does not re-verify sig — tx layer's job, `refund.rs:19–20`) | `arm_produces_a_signed_script_path_refund`, `sh_offline_after_broadcast_watchtower_covers` |
| G2 part 1 (runway): broadcast only with runway to S+Δ_early−Δ_buffer | v3.13 §Settlement, §Deadline b2 | `state_machine.rs:879`, runway `:891–897` | implemented (confirmation depth folded into Δ_buffer — see 4) | `two_party_exchange_extract_and_claim` |
| G2 part 2 (armed watchtower): unforgeable receipt, not a bool | v3.13 §Settlement, §Deadline b2 | `broadcast_completion` takes `&WatchtowerReceipt`, `matches():898–902`; receipt non-Clone/private, sole ctor `confirm_watchtower_handoff:105` (fingerprint echo `refund.rs:80`) | implemented (proves "a tower ack'd this refund", not "a live tower will fire" — see 4) | `watchtower_receipt_requires_matching_echo` |
| Completion-supersedes: before refunding, re-check completion (confirmed **or** mempool) → don't refund, take swap | v3.13 §Refund subroutine b1–2 | `refund.rs:132` `should_refund`; `:146` `completion_status_of`; `:167` `run` (re-reads every entry `:157–166`) | implemented (InMempool treated as winning → extract; caller routes to extraction) | `completion_supersedes_decision_table`, `refund_completion_race_resolves_deterministically` |
| Watchtower InMempool handling: only **confirmed** is terminal; evicted completion must not stand tower down | v3.13 §Deadline b3 | `refund.rs:217` `Watchtower::poll` (Confirmed→terminal; InMempool→transient keep-watching; Unspent&matured→fire) | implemented (correct disposition asymmetry vs `run()` — see notes) | `watchtower_waits_through_mempool_completion_then_fires_on_eviction` (named regression) |
| Deterministic race reconciliation; wallet never self-double-spends | v3.13 §Refund b5 | chain layer: `chain/mod.rs:521` `broadcast` (rejects 2nd spend of confirmed `:298–300`, CSV maturity `:264–281`); surfaced `refund.rs:179` | implemented (SIM physics, not consensus) | `double_spend_of_confirmed_output_is_rejected`, `refund_completion_race_resolves_deterministically` |
| Self-verifying dual-source chain view; SV source AUTHORITATIVE; proceed-gate disagreement → wait-or-abort | v3.13 What's-New; Threat "eclipse" | `chain/mod.rs` `ChainSource`/`is_self_verifying`; `DualSourceChainView` (`new` needs ≥1 SV); `verified_*` Err on disagreement (the proceed-to-sign gate → `funding_height` waits); `authoritative_tip_height`/`authoritative_spend_status` (the self-verifying source's reading, used by the refund/watchtower path so a liar cannot strand a matured refund) | **partial** — `is_self_verifying` is a **bool label**; no real BIP157/158 filter or PoW-header validation (see 5) | `dual_source_requires_at_least_one_self_verifying`, `eclipse_all_api_path_is_defeated_by_the_self_verifying_source`, `await_funded_refuses_under_eclipse_but_proceeds_on_agreement`, `lying_source_cannot_suppress_a_matured_refund` |
| Self-limiting third-party watchtower delegated-**claim** bundle (t-gated ŝ(Comp→SL), trigger, per-swap delegation key, TEK-wrapped) | v3.13 Delegation bundle; v3.16 Req5 | **own-device SH-side refund tower present** (`refund.rs:186` `Watchtower`/`poll`); delegated SL-side claim bundle **deferred/unbuilt** | **partial** (own-device only, per Req5 frozen scope) | SH-side fully tested; **no delegated-claim / delegation-key test** |
...
2. **Is the defense-in-depth sufficient?** `validate()` at `params.rs:209–211` requires `delta_buffer > cofunding_window` so the SH broadcast buffer absorbs the same skew on the Comp→SH side. Confirm this is adequate **given the buffer is also consumed by confirmation depth** (see 4.4).
...
Related maturity-bookkeeping subtlety: `PreArmedRefund::arm` (`refund.rs:57`) computes `csv_maturity_height = s_height + csv_blocks` using `S` as base. Under skew this over-grants the *pre-armed refund's* ETA — but this is only a wallet-side "when to attempt" hint; the chain layer independently rejects a premature spend (`chain/mod.rs:272`), so it cannot cause a consensus-invalid broadcast, only a mistimed attempt. Reviewer should still confirm `arm()` callers pass per-escrow funding height when skew is possible.

### 4.2 Role-seed LSB → party-bit choice

`state_machine.rs:389–395`:
```
we_are_a    = our_session_pubkey < their_session_pubkey;
seed_picks_a = (seed[31] & 1) == 0;
role = SH  iff  we_are_a == seed_picks_a;
```
The spec says "the least-significant bit assigns SH" but **does not pin (a) LSB of which byte under which endianness, nor (b) which parity selects A vs B.** The code takes the LSB of the last byte of the big-endian SHA-256 and maps `LSB==0 → canonical user A is SH`. The scaffold self-flags this (`state_machine.rs:335–337,381–385`: "a documented stand-in"). Both wallets running *this* code agree (antisymmetric; tested `failure_checklist.rs:1302`), so it is internally consistent — but it is a **unilateral convention not ratified against an authoritative spec bit-mapping.**
...
### 4.3 Concurrent-session interlock — wire-level enforcement (adversarial mismatch test now present)

The interlock *primitive* is evidence-in-the-type (`public_nonce` readable only via `commit_and_reveal`, which needs both sibling sessions sharing one `Rc<SingleSignerLease>`; `sign_partial` hard-blocks before the flag is set). The *wire ordering* — both parties' commitments exchanged before either reveal, and the counterparty's revealed nonces re-hashed against their prior commitment — lives in the driver (`state_machine.rs:485–509`, mismatch → Err at `:505`).

**Reviewer should confirm** the driver matches the counterparty's commitment against the revealed nonces before aggregation (it does, at `:505`). The commitment-mismatch branch **is exercised adversarially** by `failure_checklist.rs::equivocated_nonce_reveal_is_rejected` (`:829`): an `Equivocator` transport passes the counterparty's commitment through but substitutes a *different* valid `Nonces` reveal, and the victim aborts with `Error::Verification` before signing — the concurrent-session-attack property, proven negatively.

### 4.4 Deadline discipline — is Δ_buffer alone the right encoding of confirmation runway?

The G2 runway check is a pure height comparison (`current_height >= s_height + delta_early - delta_buffer → refuse`, `state_machine.rs:891–897`, `>=` so the boundary height is itself refused). The spec's "enough blocks to reach confirmation" is **folded into Δ_buffer sizing** rather than modeled as a separate confirmation-depth parameter. Since Δ_buffer is *also* required to exceed `cofunding_window` (4.1), it is doing double duty: skew absorption **and** confirmation runway.
...
Related: `broadcast_completion` returns the `CompletionSig` but **does not actually broadcast** — the Tor multi-peer broadcast + confirmation babysitting + fee bump is a documented stub (`state_machine.rs:904–905`). The deadline *arithmetic* is real; the network fill is deferred.

### 4.5 Possession gate G1 — liveness on undeliverable enabling partial

On release send-error (`state_machine.rs:627–631`) the code **deliberately does not unwind to refund**: it treats delivered-or-not as unknowable (TCP/Tor) and proceeds as released, relying on the persisted record. This is the correct crash-safe choice, but it means a genuinely-never-delivered enabling partial leaves SL committed to the extraction path while SH cannot complete — **both then refund via timelocks.** Consistent with spec L129 ("every intermediate state is non-broadcastable"). **Reviewer should confirm the liveness reasoning.**

Also: G1's binding-to-message (`msg_comp_sh`) is *recorded* (`:143`, dead_code) for audit, but the operative runtime check keys on **swap_session_id** (`:308`). Confirm swap-id binding is sufficient and that message binding need not be runtime-enforced.
...
exactly the scheme-(a) tier escrow amount — D+Δ_fee−setup_cost,
`expected_escrow_amount` — before signing) + the tight co-funding window + Block-X are the
...
| **Setup-tx construction (zero-change)** | **implemented** | `tx::setup::build_setup` spends the whole D+Δ_fee pre-encumbrance UTXO (Taproot single-sig, `pre_encumbrance_spk`) into the escrow with NO change — single input, escrow output + non-dust P2A anchor, TRUC/v3, key-path signed with the funder's taproot-tweaked key. The SimChain now registers a confirmed tx's outputs as spendable, so `full_swap_from_real_setup_funding` runs the whole chain: pre-encumbrance → SL-first Setup → real escrow outpoint → adaptor swap → both completions at exactly D. | LOW — confirm the scheme-(a) Setup fee model (positive baked `setup_fee` ⇒ standalone relay; anchor CPFP strictly congestion-only) and the whole-UTXO/no-change arithmetic (escrow output = D+Δ_fee − setup_fee − anchor, §4.98). |
| **SimChain does not run script / verify sigs** | **partial (deliberate, documented)** | SimChain models consensus **physics** (single-spend, CSV maturity, RBF, congestion min-fee) **plus, since §4.98, Core relay POLICY** (`chain/policy.rs`: dust, standardness, TRUC/BIP431 topology, ephemeral-dust, 1P1C package feerate — enforced by `broadcast`/`submit_package`) so the atomicity/ordering/fee rows are meaningful — but **does not execute Bitcoin Script or verify signatures.** Any test broadcasting a `[0u8;64]` placeholder witness exercises **outpoint/timelock logic only.** Real BIP340 key-path validity is proven **only** in `tests/taproot_swap.rs` (bitcoin-side `verify_schnorr`) and independently in `state_machine.rs:1015`. **No real bitcoind/testnet broadcast has occurred.** | MEDIUM — "confirmed in SimChain" is **not** evidence of a valid signature; the two proofs (physics vs sig-validity) live in different files, neither alone a full-node validation. |
...
- `state_machine.rs:1116` `two_party_exchange_extract_and_claim` — full ordered six-message exchange between two real parties over in-memory duplex; assemble+verify both legs; **independent BIP340 verification** of both completed signatures (`independently_verify_leg:1015`); proves extracted `t` is correct because a wrong `t` could not produce a verifying signature (`:1153`); asserts claim lands before `S + Δ_late` for the actual sampled delay.

**G1 possession gate:**
- `state_machine.rs:1162` `sl_aborts_without_releasing_on_corrupt_sh_partial` — **definitive G1 negative**: `CorruptingTransport` flips a byte; SL aborts, never sends tag-0x04 (`:1251`), writes no possession record (`:1253`).

**Crash safety:**
- `state_machine.rs:1264` `sl_restores_possession_after_crash_and_claims` — drops in-memory `Possessing`, rebuilds from the **sealed** on-disk record in a fresh process, extracts + claims.
- `:1300` `corrupt_possession_record_is_rejected` — flipped ciphertext byte → GCM auth fails → restore Err (tamper never reaches the parser).
...
- `signing/mod.rs:390–412` `real_crash_leaves_lease_held_and_refuses_signing` — crash via `mem::forget`; restart refused.
...
- `state_machine.rs:1351` `extraction_rejects_unrelated_final_sig` — foreign swap's final sig → Err; own → Ok.

**Nonce lifecycle & interlock:**
- `signing/mod.rs:352,376` — lone/mismatched sessions rejected; signing before interlock refused.
- `:415–424` `fresh_nonce_every_session_even_same_inputs`.
- `:337–348` lease exclusivity.
...
**Taproot / validation:**
- `escrow.rs:186–205` p2tr + tweak equality; `csv_out_of_16bit_range_is_rejected`.
...
- `chain/mod.rs` `double_spend_of_confirmed_output_is_rejected`, `csv_spend_is_rejected_until_matured`, `congestion_rejects_low_fee_and_accepts_bumped`, `higher_fee_replaces_mempool_incumbent_lower_does_not`; dual-source eclipse suite (`dual_source_requires_at_least_one_self_verifying` … `eclipse_all_api_path_is_defeated_by_the_self_verifying_source`, `await_funded_refuses_under_eclipse_but_proceeds_on_agreement`).

**Real BIP340 signature validity (separate from SimChain physics):**
- `tests/taproot_swap.rs:157,168` `verify_taproot_key_spend` (bitcoin secp256k1-0.29 side) — the only place a completed MuSig2+adaptor signature is proven a valid Taproot key-path witness.

**Notable test gaps (cross-referenced to their Section 3 rows):**
1. No rogue-key attack test (property inherited from library audit, not demonstrated) (3.4).
2. No escrow-orientation assertion (SL-escrow=Δ_early / SH-escrow=Δ_late is unwired) (3.3).
3. No on-disk-absence test for the secret nonce — guarantee is by-construction; `failure_checklist.rs:286–288` asserts lease-dir emptiness after session end as a partial empirical backstop, but no test scans persisted bytes for nonce material (3.2).

**Closed since first synthesis** (kept for audit continuity; each verified against code 2026-07-09):
- ~~No co-funding-skew (skew ≠ 0) test~~ — now covered at two levels: `cofunding_skew_anchors_claim_to_swept_escrow_not_s:737` and `full_swap_under_cofunding_skew:938` drive real `await_funded` at skew=cofunding_window, assert the claim anchors to the swept escrow's own height and confirms inside `f_sh + Δ_late`, with a regression guard that the old S-anchored bound would have breached (4.1).
- ~~No adversarial commitment-mismatch test~~ — `equivocated_nonce_reveal_is_rejected:829` passes the commitment through unchanged and substitutes a *different* valid reveal; the victim aborts `Error::Verification` at `state_machine.rs:505` before any signing or aggregation (4.3).
- ~~No completion `output[0].value == D` assertion~~ — asserted for BOTH completions, pinned to index 0: the helper reads `tx.output[0].value` (`failure_checklist.rs:218–221`), asserted == tier_d in the happy path (`:246–251`) and the taproot on-chain test (`:1230–1231`), plus exact-D construction/post-CPFP invariants (`:646–647`, `:678–679`) (3.6).

---

*Reviewer's shortest path:* read Section 4.1 (timelock anchoring) and 4.8 (library identity) first — they gate the composition's soundness and the very meaning of the adaptor audit; then the G1 flow in `state_machine.rs:455–648` against 3.1; then Section 5 to bound what the tests actually prove. The atomicity argument is only as strong as (a) the `f_sh` anchoring being correct and reconciled with the spec, and (b) conduition/musig2's internal `(R+T,P,m)` verification being genuine.